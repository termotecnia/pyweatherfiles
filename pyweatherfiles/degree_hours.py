...
- :class:`EpwGroupTrendAnalyzer` — runs :class:`DegreeHoursCalculator` over
  an entire *set* of EPW files (e.g. a whole folder of multi-year records),
  automatically classified into named groups (e.g. one per city/climate)
  from each filename via a configurable regex, and adds per-group
  year-over-year trend statistics plus small-multiples plotting — ideal for
  "20 years x N climates, never pooled together" analyses.
...
class EpwGroupTrendAnalyzer:
    ...
    Unlike :class:`EpwBatchAnalyzer` — which compares a handful of
    individually-named EPWs (e.g. "TMY vs. official file vs. one real
    year") in a single MultiIndex table — this class targets the opposite
    situation: *many* EPWs covering *several independent climates* over
    *many years each* (e.g. ``longterm_epw/granada_2005.epw`` …
    ``longterm_epw/seville_2025.epw``), where each climate must be analysed
    on its own — never pooled/averaged with another — to expose a genuine
    multi-year trend (as used for Section 3.1 of the manuscript).
