...
class TMYGenerator(
    ...
    Attributes:
        ...
        df_daily (pandas.DataFrame or None): Daily source data / aggregates
            after Step 1.
        source_years (list[int] or None): Calendar years actually present in
            the **source file**, recorded by Step 1 before the resampling
            rebuilds a strict, gap-free grid (so that
            :meth:`validate_step_1_data_loading` can report which years were
        ...
        validation_step1_coverage (pandas.DataFrame or None): Coverage/
            completeness audit of ``df_hourly``/``df_daily`` (populated by
            :meth:`validate_step_1_data_loading`).
        validation_step1_hourly_stats (pandas.DataFrame or None):
            ``df_hourly.describe().T`` — one row per variable (idem).
        validation_step1_daily_stats (pandas.DataFrame or None):
            ``df_daily.describe().T`` — one row per variable (idem).
        ...
        validation_step6_tmy_final_stats (pandas.DataFrame or None):
            ``tmy_final.describe().T`` — one row per variable (populated by
            :meth:`validate_step_4_final_tmy`).
    ...
    def __init__(self, file_path, cdf_method='daily', years_to_include=None, weights=None,
        ...
        self.df_hourly = None
        self.df_daily = None
        self.source_years = None  # Calendar years found in the source file (Step 1)
        ...
        self.validation_step1_coverage = None      # Generated by validate_step_1_data_loading()
        self.validation_step1_hourly_stats = None  # idem (df_hourly.describe().T)
        self.validation_step1_daily_stats = None   # idem (df_daily.describe().T)
        ...
        self.validation_step6_tmy_final_stats = None  # Generated by validate_step_4_final_tmy()
