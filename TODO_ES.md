...
- **Lo realizado:**
  - [x] Se eligió **Sphinx** (+ `myst-nb` para páginas Markdown *y* para renderizar el notebook tutorial, + tema Furo), coherente con lo que `dist_build_docs.bat` ya esperaba. Se recrearon `docs/source/conf.py`, `docs/Makefile`, `docs/make.bat`, y el árbol de páginas (`index.md`, `installation.md`, `quickstart.md`, `tutorial.md`). Se añadió un extra `docs` en `pyproject.toml` (`pip install -e ".[docs]"`). `docs/source/api/` y `docs/source/tutorial_pyweatherfiles.ipynb` se regeneran automáticamente en cada build (`sphinx-apidoc` / copia desde `examples/`) pero se versionan igualmente como backup en GitHub; solo `docs/build/` (el HTML final) queda excluido de git.
