...
**Estado de la Fase 5 tras esta pasada:** `degree_hours.py` y `epw_trend_analyzer.py` completados; **solo `tmy.py` queda pendiente** (el de mayor riesgo — una única clase monolítica de 4.100 líneas, sin ningún test que la cubra todavía; se recomienda no abordarlo sin antes añadir al menos un smoke test de `generate_tmy()` end-to-end con datos sintéticos, igual que se hizo aquí antes de tocar cada módulo). Suite completa: 70/70 tests en verde.
...
| 6 | Fase 5 | 🟡 En progreso (`degree_hours/` y `epw_trend_analyzer/` completados; solo `tmy.py` pendiente) | Mayor beneficio a largo plazo para mantenibilidad, pero mayor riesgo — condicionada a tener tests. |
