...
> Nota (tras Fase 5/6): la lista de "sin cubrir todavía" de arriba quedó desactualizada — `tmy/` (paquete), `degree_hours/` (paquete), `epw_trend_analyzer/` (paquete) y `climate_processor.py` ya tienen smoke tests/tests de regresión (ver Fases 5 y 6), y `epw_comparator.py` se cubrió en el punto 7 justo arriba. Cobertura por módulo medida con `pytest --cov` tras esta pasada (de mayor a menor hueco): `tmy/_plotting.py` 23%, `degree_hours/calculator.py` 24%, `degree_hours/_helpers.py` 35%, `tmy/_data_loading.py` 40%, `degree_hours/group_trend_analyzer.py` 46%, `tmy/_validation.py` 49%, `session_manager.py` 50%, `hourly_epw_converter.py` 55%, `degree_hours/batch_analyzer.py` 59%, `met_epw_converter.py` 64% — candidatos naturales para continuar esta fase en el futuro, sin urgencia (todos tienen ya al menos cobertura indirecta vía los smoke tests end-to-end de sus respectivos paquetes).
