...
- Validation: `python -m pytest tests/test_epw_comparator.py -q` → 12 passed; `python -m pytest tests/test_met_epw_converter_extra.py -q` → 20 passed; workflow YAML parses and the step list is `checkout@v5 / setup-python@v6 / install / pytest / upload-artifact@v4`; CI run `35571739471` proved the collection fix (386 passed vs 0 before) and isolated the single remaining failure, addressed above.
