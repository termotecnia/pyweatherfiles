...
- `pyweatherfiles/met_epw_converter.py`: function-style MET<->EPW conversion (`convert_met_to_epw`, `convert_epw_to_met`). Shares non-trivial duplicated logic with `hourly_epw_converter.py` (barometric pressure formula, Ladybug point-in-time offset correction, the 15-field "unused by EnergyPlus" map, `Sunpath`-based DHI/DNI reconstruction) — see `INFORME_REVISION_GENERAL.md` §3.1 before touching either.
