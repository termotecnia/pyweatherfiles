...
- `pyweatherfiles/degree_hours.py` (~2.6k lines): `DegreeHoursCalculator` (single EPW) + `EpwBatchAnalyzer` (multi-EPW comparative tables, MultiIndex columns) + `EpwGroupTrendAnalyzer` (whole-folder degree-hours trend analysis, classified by filename via `epw_utils.classify_epw_files`; also exposes `fit_global_trend()`, a global fixed-effects trend sharing the exact estimator used by `EpwTrendAnalyzer.fit_global_model()` via `trend_stats.py` — Fase 4, now done).
- `pyweatherfiles/epw_trend_analyzer.py` (~1.7k lines): `EpwTrendAnalyzer`/`TrendConfig`/`OutputConfig` — multi-year temperature-trend analysis (warming, heatwaves) with a global fixed-effects model across cities. Still conceptually overlaps with `EpwGroupTrendAnalyzer` (both classify/trend EPW collections by city+year, different domains: temperature vs. degree-hours) but both now share the same file-classification helper (`epw_utils.classify_epw_files`, via `discover_files()`) and the same global-trend estimator (`trend_stats.fit_fixed_effects_model`) — see `INFORME_REVISION_GENERAL.md` §3.3/Fase 4 (now done) and README.md §7.4 for a feature-comparison table / "which one to use" guidance before adding features to either.
...
- **Tests**: `tests/` now exists (pytest, extra `test` in `pyproject.toml`: `pip install -e ".[test]"` then `python -m pytest tests/`). 58 tests as of Fase 4 (see `INFORME_REVISION_GENERAL.md` §6 Fase 2/Fase 4) covering the pure physical/EPW-plumbing functions, the two EPW-writing pipelines, `trend_stats.py`, `epw_utils.classify_epw_files`, and the `EpwTrendAnalyzer.discover_files()`/`EpwGroupTrendAnalyzer.fit_global_trend()` integration; `tmy.py`/most of `degree_hours.py`/`epw_trend_analyzer.py` core logic is still otherwise untested.
