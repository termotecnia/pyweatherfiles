...
- **Tests**: `tests/` now exists (pytest, extra `test` in `pyproject.toml`: `pip install -e ".[test]"` then `python -m pytest tests/`). 64 tests as of Fase 5 (see `INFORME_REVISION_GENERAL.md` §6 Fase 2/Fase 4/Fase 5) covering the pure physical/EPW-plumbing functions, the two EPW-writing pipelines, `trend_stats.py`, `epw_utils.classify_epw_files`, the `EpwTrendAnalyzer.discover_files()`/`EpwGroupTrendAnalyzer.fit_global_trend()` integration, and end-to-end smoke tests of the modularized `degree_hours` package; `tmy.py`/`epw_trend_analyzer.py` core logic is still otherwise untested.
